...
try, it = tc.getIterations(); item.iterations = numel(it); catch, item.iterations = 0; end
try, ass = tc.getAssessments(); item.assessments = numel(ass); catch, item.assessments = 0; end
try, bc = tc.getBaselineCriteria(); item.baseline_criteria = numel(bc); catch, item.baseline_criteria = 0; end
end
